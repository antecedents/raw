...
class Inspect:
    ...
    def __attendance_category(self):
        ...
        assert tensor.shape[0] == 1, 'The # of attendance categories is > 1; there should be 1, i.e., Unplanned, only.'
        assert np.all(np.char.equal(tensor,
                                    c_attendance_category)), f'Invalid attendance category.  The attendance category of each instance should be {c_attendance_category}'
